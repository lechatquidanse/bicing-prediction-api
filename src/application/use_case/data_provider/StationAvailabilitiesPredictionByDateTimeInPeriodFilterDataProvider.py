...
class StationAvailabilitiesPredictionByDateTimeInPeriodFilterDataProvider:
    ...
    def collection(self, station_id: uuid,
                   filter: ByDateTimeInPeriodFilter) -> StationAvailabilitiesPredictionByDateTimeInPeriodFilterView:
        station_availability_algorithm = self._repository.find_by_station_id(station_id)

        if None == station_availability_algorithm:
            raise StationAvailabilityAlgorithmDoesNotExistException.with_station_id(station_id)

        prediction_data_set = StationDataPrediction(filter.to_date_time_index(),
        ...
        return StationAvailabilitiesPredictionByDateTimeInPeriodFilterView(station_id, filter, predictions)
