...
def feature_importance_plot(importance_sorted, title):
    ...
    plt.tight_layout()
    plt.savefig('/var/www/bicing-prediction/plots/xgb/' + title + '.png', dpi=300)
    # plt.show()


def xgb_importance(df, test_ratio, xgb_params, n_tree, early_stop, plot_title):
    df = pd.DataFrame(df)
    ...
    xgb_model = xgb.train(xgb_params, dtrain, n_tree, evals=watchlist, early_stopping_rounds=early_stop,
                          verbose_eval=False)
...
def xgb_forecasts_plot(plot_start, Y, Y_test, Y_hat, forecasts, title):
    ...
    ax.fill_betweenx(ax.get_ylim(), pd.to_datetime(Y_test.index[0]), Y_test.index[-1],
                     alpha=.1, zorder=-1)
    ax.set_xlabel('Time')
    ax.set_ylabel('Available Bike Number')
    plt.legend()
    plt.tight_layout()
    plt.savefig('/var/www/bicing-prediction/plots/xgb/' + title + '.png', dpi=300)
    plt.show()
