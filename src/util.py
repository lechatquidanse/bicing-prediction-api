...
def pre_process(n_rows, parse_dates, filename):
    ...
    df = pd.read_csv(filename, header=0, delimiter=',', sep='', names=variable_names.columns,
                     parse_dates=parse_dates, index_col='stated_at', nrows=n_rows, skiprows=total_rows - n_rows)
...
def timeseries_plot(y, color, y_label):
    ...
    plt.tight_layout()
    plt.savefig('/var/www/bicing-prediction/plots/util/' + y_label + '.png', dpi=300)
    plt.show()
...
def get_unseen_data(unseen_start, steps, encode_cols, bucket_size):
    ...
    return pd.DataFrame(pd.Series(np.zeros(steps), index=index), columns=['available_bike_number'])
